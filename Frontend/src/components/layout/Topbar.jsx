...
import {
  RiSearchLine, RiAddLine, RiLinkM, RiUpload2Line,
  RiFolderAddLine, RiPriceTag3Line, RiMoonLine, RiSunLine,
} from "react-icons/ri";
...
export default function Topbar() {
  ...
  return (
    <>
      <header className="h-14 border-b border-border flex items-center gap-3 px-4 bg-background/80 backdrop-blur shrink-0">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-44">
            <DropdownMenuItem onClick={() => setModal("save")} className="gap-2">
              <RiLinkM size={14} /> Save URL
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => setModal("upload")} className="gap-2">
              <RiUpload2Line size={14} /> Upload from PC
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => setModal("collection")} className="gap-2">
      ...
      <UploadFileModal isOpen={modal === "upload"} onClose={() => setModal(null)} />
      <CreateCollectionModal isOpen={modal === "collection"} onClose={() => setModal(null)} />
